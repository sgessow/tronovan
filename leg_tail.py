...
class Robot_Part(Framework):
    ...
    def __init__(self,args=None):
        ...
        super(Robot_Part, self).__init__()
        self.count=0
        self.points=[]
        self.group=-1
        ...
        self.vertical = self.world.CreateDynamicBody(
            position=(0, self.start_y),
            fixedRotation=True,
            fixtures=b2FixtureDef(
                shape=b2PolygonShape(box=(torso_width, self.len_torso/2)),density=1.3,filter=b2Filter(groupIndex=self.group,)),
        ...
        self.stability_joint=self.world.CreateRevoluteJoint(
                    bodyA=self.vertical,
                    bodyB=self.torso,
                    anchor=(0,self.start_y-self.len_torso/2),
                    enableLimit = True,
                    lowerAngle=-numpy.pi/4,
                    upperAngle=numpy.pi/4,
                    enableMotor=True,
                    maxMotorTorque =0.0001,
                    motorSpeed=0,
        )
    ...
    def Step(self, settings):
        ...
        super(Robot_Part, self).Step(settings)
        # do stuff
        if(self.count%5==0 and False):
            ...
            #     self.world.DestroyBody(self.points[0])
            #     self.points.pop(0)
        else:
            self.count+=1
...
class Robot(Framework):
    """You can use this class as an outline for your tests."""
    name = "Robot"  # Name of the class to display
    description = "Simple Walking Robot 4 legs"
...
if __name__ == "__main__":
    main(Robot_Part,{"start_y":.1,"len_torso":.03,"len_crank_arm":.01,"len_leg":.06,"motor_torque":10, "motor_speed":2})
